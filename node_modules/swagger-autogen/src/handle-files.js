        ...
        fs.readFile(filePath, 'utf8', async function (err, data) {
            ...
            if (regex != '' || aForcedsEndpoints.length > 0) {
                ...
                for (let idxElem = 0; idxElem < aData.length; idxElem++) {
                    ...
                    if ((elemParam && elemParam.split(',').length > 1 && !forced) || predefMethod === 'use') {
                        ...
                        for (let index = 0; index < functions.length; index++) {
                            ...
                            if (idx > -1 || exportPath) {
                                 ...
                                 */
                                if(aDataRaw.includes('import { Router } from \'express\';\n\nimport TestController from \'@modules/test/infra/controller')){
                                    console.log()
                                }
                    ...
                    /**
                     * Try to find referenced returns
                     */
                    for (let index = 0; index < endpointFunctions.length; index++) {
                        let func = endpointFunctions[index].func;
                        let regexGetReturns = /[^a-z|^A-Z|^0-9|^_|^$]return[^a-z|^A-Z|^0-9|^_|^$]/
                        if (func && regexGetReturns.test(func)) {
                            let returns = func.split(new RegExp(regexGetReturns))
                            returns.shift();
                            for (let retIdx = 0; retIdx < returns.length; retIdx++) {
                                let _return = returns[retIdx].trim().split(new RegExp(/\s|\n|\t|\r|\;/))[0];
                                // TODO: check if it is a primitive, such as: number, string, boolean, ...
                                // TODO: Try do find in other file
                                
                                /**
                                 * Try to find in the same file 
                                 */ 

                                // Check if it is a variable
                                let solvedFunction = false;
                                if (jsParsed && jsParsed.variables.length > 0) {
                                    let found = jsParsed.variablesfilter(v => v.name == _return && v.end <= bytePosition);
                                    console.log(bytePosition)
                                } 
