const kakaologin = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const kakaoEmail = await userRepository.findOne({
    ...
    if (kakaoEmail && !kakaoEmail.kakao) {
      return res
        .status(409)
        .redirect(`${process.env.CLIENT_URI}/login?islogin=fail`);
    }
    if (kakaoEmail && kakaoEmail.kakao) {
      if (new Date(kakaoEmail.penalty).getTime() - Date.now() > 0)
        return res
          .status(400)
          .redirect(
            `${process.env.CLIENT_URI}/callback?ban=${kakaoEmail.penalty}`
          );
      ...
      return res
        ...
        .cookie(
          'accessToken',
          accessToken /* , {domain: 'newb-d.com', sameSite: 'none', secure: true} */
        )
    ...
    while (true) {
      const kakaoNickname = await userRepository.findOne({
        nickname: nickname,
      });
